            ...
            if (this.status == EntitySignalStatus.Disconnected) {
                ...
                this.connectingDefer = new Promise(function (resolve, reject) {
                    ...
                    _this.hub.start().then(function (x) {
                        ...
                        setTimeout(function () {
                            if (_this.status == EntitySignalStatus.WaitingForConnectionId) {
                                reject();
                            }
        ...
        Client.prototype.processSync = function (data) {
            ...
            data.urls.forEach(function (url) {
                ...
                url.data.forEach(function (x) {
                    // Alchi -> PascalCase ----- Start
                    for (var prop in x.object) {
                        if (x.object.hasOwnProperty(prop)) {
                            x.object[prop.substring(0, 1).toUpperCase() + prop.substring(1)] = x.object[prop];
                            delete x.object[prop];
                        }
                    }
                    _this.options.defaultId = _this.options.defaultId.substring(0, 1).toUpperCase() + _this.options.defaultId.substring(1);
                    // Alchi -> PascalCase ------ End
                    // Alchi
                    if (typeof pageCrudItem === "function") {
                        pageCrudItem(x.typeName, x.state, x.object, _this.options.defaultId);
                    }
                    ...
                    xhr.onreadystatechange = function () {
                        if (xhr.readyState == 4) {
                            if (xhr.status == 200) {
                                ...
                                console.log('Error: ' + url + ' - ' + xhr.status); // Alchi
                            }
                        }
                    };
                    xhr.send();
